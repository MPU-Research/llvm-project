...
void RISCVDAGToDAGISel::selectMPEArith(SDNode *Node, bool IsMasked = false) {
  ...
  if (IsMac) {
    SDValue Acc = Node->getOperand(3);
    assert((LHS.getValueType() == Acc.getValueType()) &&
           "Acc must have same MVT as LHS!");
  ...
  if (!IsMasked) {
    ...
    SDValue Mask = Node->getOperand((IsMac) ? 4 : 3);
...
void RISCVDAGToDAGISel::selectMPEMove(SDNode *Node, bool IsMasked = false) {
  ...
  case Intrinsic::riscv_mpe_move_xm_mask:
    Opcode = RISCV::MMV_XM;
    break;
  case Intrinsic::riscv_mpe_move_xz:
  case Intrinsic::riscv_mpe_move_xz_mask:
